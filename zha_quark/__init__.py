...
class InovelliVZM32SNMMWaveCluster(CustomCluster):
    ...
    class ServerCommandDefs(BaseCommandDefs):
        """Client-to-Server commands (sent from HA to the mmWave sensor)."""
    ...
    class ClientCommandDefs(BaseCommandDefs):
        """Server-to-Client commands (reports sent from the mmWave sensor to HA)."""

        anyone_in_area = ZCLCommandDef(
            id=0x00,
            schema={
                "area1": t.uint8_t,
                "area2": t.uint8_t,
        ...
        report_interference_area = ZCLCommandDef(
            ...
            schema={
                "count": t.uint8_t,
                "x_min_1": t.int16s, "x_max_1": t.int16s,
                "y_min_1": t.int16s, "y_max_1": t.int16s,
                "z_min_1": t.int16s, "z_max_1": t.int16s,
                "x_min_2": t.int16s, "x_max_2": t.int16s,
                "y_min_2": t.int16s, "y_max_2": t.int16s,
                "z_min_2": t.int16s, "z_max_2": t.int16s,
                "x_min_3": t.int16s, "x_max_3": t.int16s,
                "y_min_3": t.int16s, "y_max_3": t.int16s,
                "z_min_3": t.int16s, "z_max_3": t.int16s,
                "x_min_4": t.int16s, "x_max_4": t.int16s,
                "y_min_4": t.int16s, "y_max_4": t.int16s,
                "z_min_4": t.int16s, "z_max_4": t.int16s,
            },
            is_manufacturer_specific=True,
        )
    ...
    @staticmethod
    def _parse_area_report(args) -> dict:
        """Parse a full 4-area report into a structured dict (matches Z2M naming)."""
        return {
            "count": args.count,
            **{
                f"area{i}": {
                    "width_min": getattr(args, f"x_min_{i}"),
                    "width_max": getattr(args, f"x_max_{i}"),
                    "depth_min": getattr(args, f"y_min_{i}"),
                    "depth_max": getattr(args, f"y_max_{i}"),
                    "height_min": getattr(args, f"z_min_{i}"),
                    "height_max": getattr(args, f"z_max_{i}"),
                }
                for i in range(1, 5)
            },
        }
    ...
    def handle_cluster_request(
        ...
        *,
        ...
        """Handle cluster commands from the mmWave sensor."""
        if hdr.command_id == 0x00:  # anyone_in_area
            event_args = {
                "area1": args.area1,
                "area2": args.area2,
                "area3": args.area3,
            ...
            event_name = {
                ...
                0x03: "mmwave_report_detection_area",
                0x04: "mmwave_report_stay_area",
            }[hdr.command_id]
    ...
    async def bind(self):
        ...
        try:
            await self.request(
                ...
                self.ServerCommandDefs.mmwave_control_command.id,  # 0x00
                self.ServerCommandDefs.mmwave_control_command.schema,
                control_id=2,  # query_areas — triggers the device to start reporting
                expect_reply=False,
